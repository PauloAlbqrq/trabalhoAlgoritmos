...
void empilhar(pilha *stack, const char *item) {
  if (stack->apontador < TMN_MAXIMO - 1) { // verifica se a pilha não está cheia
    ...
    strncpy(stack->pilhaArray[stack->apontador], item,
            50); // copia o item para a pilha
    stack->pilhaArray[stack->apontador][49] =
        '\0';         // garante que a string termina com \0
...
char *desempilhar(pilha *stack) {
  if (stack->apontador >= 0) { // verifica se a pilha não está vazia
    char *retorno =
        stack->pilhaArray[stack->apontador]; // salva o item do topo que será
                                             // desempilhado
...
void exibirHistorico(pilha *stack) {
  for (int i = 0; i <= stack->apontador;
       i++) {                             // percorre todos os itens da pilha
...
void menu(pilha *stack) {
  ...
  while (loop) {    // loop principal
    ...
    case 1:
      ...
      empilhar(stack,
               input); // chama a função empilhar passando a entrada do usuário
      break;
    case 2:
      printf(
          "%s \n",
          desempilhar(
              stack)); // chama a função desempilhar e exibe o item desempilhado
      break;
    case 3:
